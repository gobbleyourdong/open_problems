# Dysbiosis — gap.md Per-Run Extension Archive

**Status:** archived 2026-04-14. These blocks were originally appended to `gap.md` after each numerics/run_NNN_*.md during the /loop map-the-space campaign (Apr 11–12, covering runs 001–170). Each Extension is a derivative summary of a canonical run file in `numerics/`; the run files remain the source of truth.

Preserved verbatim for historical traceability per Maps Include Noise (Sigma v6). Do not extend this file — write new runs in `numerics/` directly.
...
### Extension 113 — TRPV4: Warm-Temperature Thermal Trigger + Keratinocyte Ca²⁺ + Gut Permeability (run_120)

**Gap confirmed:** TRPV4 and the 27–35°C "physiological warmth" thermal trigger mechanism are completely absent from all 119 prior runs. The framework covers TRPV1 (runs 015, 095; 43°C noxious heat receptor) and TRPA1 (run_093; cold/reactive electrophiles <17°C) but the channel that activates at exactly the temperature range of the most prevalent rosacea triggers — hot weather, warm rooms, hot baths, exercise heat — is entirely unrepresented.

**Core insight:** Epidemiological surveys show hot weather (#1, ~81% of patients) and warm environments are the most consistently reported rosacea trigger class. These temperatures (27–35°C) are below TRPV1's activation threshold (43°C) and above TRPA1's cold range. TRPV4 fills this gap: it activates at 27–35°C in keratinocytes and vascular endothelium via temperature, hypoosmolarity/cell swelling, and epoxyeicosatrienoic acids (EETs) generated by UV → PLA2 → AA → CYP2C8 → 5,6-EET. The EET pathway provides a second explanation for UV as rosacea trigger (complementing M3/photochemical mechanisms).

**New mechanisms added (run_120):**
- **TRPV4 warm trigger receptor in keratinocytes**: 27–35°C activation → Ca²⁺ influx → calcineurin/NF-AT + PKC/ERK/AP-1 → IL-8, VEGF-A, PGE2, COX-2 (NF-AT = new transcription factor pathway in the framework)
- **EET/UV → TRPV4**: UV → PLA2 → AA → CYP2C8/CYP2J2 → 5,6-EET → TRPV4 → Ca²⁺ → keratinocyte inflammation (explains UV trigger via receptor-mediated lipid metabolite mechanism)
- **Sweating/hypoosmolarity → TRPV4**: exercise → sweating → skin osmotic gradient → cell swelling → TRPV4 (explains exercise trigger even at moderate temperatures)
- **TRPV4 → Loop 1 amplification**: TRPV4 → Ca²⁺ → PKC → ERK → inflammatory milieu → KLK5 upregulation → LL-37 processing (4th upstream trigger for KLK5/Loop 1 cascade)
- **Gut TRPV4 → permeability**: exercise/heat → osmotic TRPV4 → RhoA/ROCK → MLC phosphorylation → tight junction contraction → paracellular permeability ↑ (thermal/osmotic mechanism extending run_059 and run_119)
- **β cell TRPV4 sensitization**: IL-1β in inflamed islets → TRPV4 threshold shift ↓ → chronic Ca²⁺ influx at body temperature → ER stress feedforward (amplification mechanism in T1DM inflammatory milieu)

**New protocol elements:**
- Quercetin timing: TRPV4 inhibitor (IC50 ~1–2 μM); recommend 500mg quercetin 60–90min pre-heat exposure — NEW mechanistic rationale beyond existing NLRP3/NF-κB role
- TRPV4 thermal avoidance thresholds: patient education — showers <26°C, avoid environments >27°C during active disease, exercise cooling protocol
- Omega-3/EPA connection: AA ↓ → EET ↓ → TRPV4 EET-activation route ↓ (adds mechanism to existing omega-3 anti-rosacea effect)
- Magnesium: Mg²⁺ reduces Ca²⁺ influx per TRPV4 opening; Mg²⁺ deficiency = TRPV4 hypersensitivity (new rationale for existing T-index Mg²⁺ component)
...
### Extension 139 — EIF2AK3 / PERK: ER Stress Kinase, eIF2α Integrated Stress Response, ATF4/CHOP β Cell Apoptosis, Wolcott-Rallison T1DM Locus

**Date:** 2026-04-12 | **Run:** run_146

**Gap confirmed:** EIF2AK3/PERK as primary subject: 0 hits. Appears as context mention in run_098/130/133/142 but never analyzed as primary. run_098 = IRE1α/XBP1s branch; PERK/eIF2α/ATF4/CHOP = orthogonal UPR branch.

**Core insight:** PERK (EIF2AK3) is the ER-localized kinase that phosphorylates eIF2α-Ser51 → integrated stress response (ISR): global translation attenuation + selective ATF4 translation. ATF4 has two outputs: (1) adaptive/resolved: GADD34 negative feedback → eIF2α dephosphorylation → survival; (2) chronic/unresolved: ATF4 → CHOP (DDIT3) → DR5/BIM upregulation + BCL2 ↓ → intrinsic apoptosis. β cells are uniquely vulnerable: highest secretory demand of any differentiated cell type (~1 million insulin molecules/minute), LIMITED GADD34 reserve → CHOP accumulates faster. EIF2AK3 = Wolcott-Rallison syndrome (biallelic loss = neonatal diabetes + skeletal dysplasia). PERK/CHOP = 24th β cell death mechanism (ER stress-driven intrinsic apoptosis, distinct from ferroptosis/pyroptosis/necroptosis). In rosacea: UV/LL-37/Demodex → PERK → CHOP → keratinocyte apoptosis = third parallel UV-driven keratinocyte death mode (alongside NLRP1/run_122 pyroptosis and SLC7A11/run_143 ferroptosis).

**New mechanisms added (run_146):**
- **Wolcott-Rallison syndrome (WRS) — EIF2AK3 neonatal T1DM**: biallelic loss → β cell constitutive ER stress → cell death; heterozygous common variants → subthreshold PERK → β cells closer to apoptotic threshold during insulitis overlay
- **ATF4/CHOP apoptotic cascade**: sustained ER stress → ATF4 → CHOP → DR5/TRAIL-R2 ↑ + BIM ↑ + BCL2 ↓ + TRB3 ↑ (Akt inhibition) → β cell intrinsic apoptosis; GADD34 feedback timer determines survival vs. death outcome
- **PERK/CHOP = 24th β cell death mechanism**: ER protein misfolding death, distinct from lipid (ferroptosis/run_143), inflammasome (NLRP3/pyroptosis), and oxidative (Fenton/run_110) mechanisms
- **Insulitis overlay**: NO (iNOS, cytokine-driven) → ER Ca²⁺ leak → misfolded protein surge → PERK hyperactivation → CHOP → death; low-PERK β cells (EIF2AK3 variants) → faster CHOP accumulation during this overlay
- **Third UV keratinocyte death mode** (rosacea): UV/LL-37/Demodex → PERK → CHOP → keratinocyte apoptosis; orthogonal to run_122 (NLRP1 pyroptosis) and run_143 (SLC7A11 ferroptosis)
- **ISRIB (integrated stress response inhibitor)**: stabilizes eIF2B octamer → translation restored despite eIF2α-P → β cell insulin biosynthesis maintained during ER stress; preclinical GSIS improvement
- **Salubrinal/guanabenz (GADD34 inhibitors)**: prolongs eIF2α-P duration → more adaptation time before translation resumes → acute ER stress survival benefit; NOD mouse protection
- **4-PBA/TUDCA PERK mechanism**: chemical chaperones now have dual UPR justification — run_098 (IRE1α/XBP1s) + run_146 (PERK reduction of misfolded protein load → eIF2α-P ↓ → CHOP ↓)

**Framework at SATURATION + 35: 146 runs.**

*Gap.md updated: 2026-04-12 | One-hundred-and-thirty-ninth extension | EIF2AK3 PERK eIF2α-Ser51 ISR ATF4 CHOP DDIT3 GADD34 PPP1R15A eIF2B ISRIB salubrinal guanabenz UPR-PERK-branch Wolcott-Rallison-neonatal-T1DM β-cell-24th-death-mechanism CHOP-DR5-BIM-BCL2 TRB3-Akt-inhibition insulitis-NO-ER-Ca2+-leak third-UV-keratinocyte-death-mode NLRP1-run122-parallel SLC7A11-run143-parallel 4-PBA-TUDCA-dual-UPR-justification mast-cell-FcεRI-ER-stress run098-IRE1α-XBP1s-orthogonal WRS-biallelic-EIF2AK3 | run_146*

---

### Extension 140 — SIRT1: Nuclear NAD⁺-Dependent Deacetylase, UCP2/IRS-2/RelA-K310 β Cell Protection, PARP1/UV/NAD⁺ Rosacea Axis, Treg FOXO3a Metabolic Gate

**Date:** 2026-04-12 | **Run:** run_147

**Gap confirmed:** SIRT1 as primary subject: 0 hits. Appears in 37 files as context mention. run_090 = SIRT3 (mitochondrial)/SIRT6 (nuclear DNA repair) — different isoforms, different substrates, different functions.

**Core insight:** SIRT1 is the nuclear/cytoplasmic NAD⁺-dependent deacetylase targeting non-mitochondrial substrates: UCP2-K222 (β cell insulin secretion), IRS-2 (β cell survival), RelA-K310 (NF-κB termination), FOXO3a (Treg metabolic support), PGC-1α (mitochondrial biogenesis). NAD⁺ = direct cofactor: when NAD⁺ is depleted (PARP1 activation, mitochondrial dysfunction, CD38 upregulation), SIRT1 activity falls regardless of protein level. In T1DM β cells: SIRT1 → UCP2 degradation → ATP ↑ → GSIS ↑ (SIRT1-KO = ~40% GSIS reduction); SIRT1 → RelA-K310 deacetylation → NF-κB duration limited in islets (additive with SETD7/RelA-K314/315, run_145). In rosacea: UV → PARP1 → NAD⁺ depletion → SIRT1 failure → RelA-K310 not deacetylated → NF-κB sustained → inflammatory amplification. ME/CFS: NAD⁺ deficit (PARP1/mitochondria/IDO1/CD38) → SIRT1 hypofunction → neuroinflammation sustained + NK/Treg metabolic dysfunction. AMPK (run_085) → DBC1 phosphorylation → SIRT1 derepression = mechanistic basis for metformin/AMPK anti-inflammatory benefit via SIRT1.

**New mechanisms added (run_147):**
- **UCP2-K222 deacetylation → GSIS**: SIRT1 → UCP2 protein degradation → proton coupling restored → ATP ↑ → KATP closed → insulin exocytosis ↑; SIRT1-KO β cells: ~3-5× UCP2 accumulation → ~40% GSIS ↓
- **IRS-2 deacetylation → β cell survival**: SIRT1 → IRS-2-K1173 → PI3K/Akt → FOXO1 nuclear export → β cell survival/mass maintenance; insulitis JNK → IRS-2-Ser phosphorylation counteracted by SIRT1
- **RelA-K310 NF-κB termination** (additive with run_145 RelA-K314/315): SIRT1 deacetylates RelA-K310 → NF-κB transcriptional activity ↓ without degradation; run_145 SETD7 methylates K314/315 → degradation; two non-overlapping mechanisms at adjacent residues
- **UV/PARP1/NAD⁺/SIRT1/NF-κB rosacea axis**: UV → DNA breaks → PARP1 → 100-200 NAD⁺ consumed per break → cellular NAD⁺ ↓ → SIRT1 ↓ → RelA-K310 not deacetylated → NF-κB sustained → IL-6/IL-8/MMP-1 → rosacea amplification; explains UV-disproportionate inflammation
- **AMPK → DBC1 → SIRT1 derepression**: AMPK phosphorylates DBC1 (endogenous SIRT1 inhibitor) → DBC1 released → SIRT1 active; metformin/AMPK anti-inflammatory benefit partially SIRT1-mediated via this circuit
- **NMN/NR as primary SIRT1 indication**: NAD⁺ precursor → SIRT1 cofactor replenishment → all SIRT1 substrates (RelA-K310, UCP2, FOXO3a, IRS-2) protected; prior framework mentions of resveratrol/NMN now have SIRT1 as primary mechanistic anchor
- **Niacinamide/SIRT1 paradox monitoring**: nicotinamide = SIRT1 product inhibitor (competitive); at doses >2g/day, tissue levels may approach Ki → SIRT1 partial inhibition; standard 4% topical/500 mg/day oral = low risk; monitor at >2g/day systemic
- **Treg FOXO3a metabolic gate (distinct from protein stability)**: SIRT1 → FOXO3a deacetylation → FOXO3a nuclear → Treg-supportive gene transcription (IL-2Rα, Foxp3) + β-oxidation genes → Treg metabolic fitness; low-NAD⁺ insulitis microenvironment → SIRT1 ↓ → FOXO3a cytoplasmic → Treg metabolic failure without FOXP3 degradation
...
### Extension 153 — RIPK1/RIPK3/MLKL Necroptosis: Necrosome Execution, β Cell DAMP Storm, Keratinocyte Loop 2, Necrostatin-1

**Date:** 2026-04-12 | **Run:** run_160

**Gap confirmed:** RIPK3/MLKL/necrosome execution machinery absent from all 159 runs as primary; run_113 mentions RIP1 as secondary output of A20 K48-Ub regulation; necrostatin-1/GSK-843/MLKL pore never addressed.

**Core insight:** Necroptosis is the **maximally immunogenic β cell death mode** — RIPK1 kinase activation → RHIM:RHIM RIPK3 interaction → necrosome amyloid fibril → MLKL-Thr357/Ser358 phosphorylation → 4HB domain pore → catastrophic simultaneous DAMP release (HMGB1 + mtDNA + ATP + IL-33 + β cell antigens). Distinct from apoptosis (quiet, efferocytosed), pyroptosis (run_096/GSDMD), and ferroptosis (run_143/GPX4). A20 haploinsufficiency (run_113) mechanistically explained: A20 ↓ → RIPK1 accumulates → necrosome enabled → β cell death shifts from quiet apoptosis to DAMP storm. Three DAMP amplification loops fired simultaneously by necroptotic β cell: TLR4/NF-κB (HMGB1), TLR9/IFN-α (mtDNA), ST2/IL-1β (IL-33), P2X7/NLRP3 (ATP), and cDC1 cross-presentation (run_159 amplified).

**New mechanisms added (run_160):**
- **RIPK1 scaffold vs. kinase dichotomy**: scaffold (IKK/NF-κB, run_113 A20 target) vs. kinase (necrosome Ser166 autophosphorylation → RHIM-RIPK3) → necrostatin-1 blocks kinase ONLY; NF-κB preserved
- **RHIM domain necrosome assembly**: RIPK1:RIPK3 amyloid fibril → ordered β-structure → RIPK3 autophosphorylation (Ser227) → MLKL phosphorylation (Thr357/Ser358) → 4HB pore
- **A20/necroptosis bridge**: A20 haploinsufficiency (run_113) → RIPK1 accumulates → necrosome enabled → apoptosis→necroptosis shift = the mechanistic explanation for why TNFAIP3 GWAS risk drives catastrophic insulitis
- **Simultaneous DAMP storm**: HMGB1 (TLR4) + mtDNA (TLR9/IFN-α) + IL-33 (ST2/run_099) + ATP (P2X7/NLRP3) + β cell antigens (cDC1/run_159) = five parallel amplification loops fired by one necroptotic β cell
- **26th β cell death mechanism**: necroptosis (RIPK3/MLKL) joins apoptosis/pyroptosis/ferroptosis as fourth membrane-disruption mode
- **Keratinocyte necroptosis → Loop 2**: UV + Demodex → RIPK3 → keratinocyte DAMP → TLR4/NF-κB priming; high-dose UV saturates caspase-8 → necroptosis default
- **ME/CFS RIPK3 elevation**: EBV vICA → caspase-8 blocked → necroptosis default in EBV-infected cells → mtDNA → TLR9 → IFN-α → M3 amplification
- **Necrostatin-1/dabrafenib**: RIPK1 kinase inhibitor (Nec-1s) and RIPK3 inhibitor (dabrafenib repurposed, Mandal 2014 Cell NOD prevention) = first necroptosis-specific therapeutics in framework

**Framework at SATURATION + 49: 160 runs.**

*Gap.md updated: 2026-04-12 | One-hundred-and-fifty-third extension | RIPK1-Ser166-kinase RIPK3-Ser227 MLKL-Thr357-Ser358 RHIM-amyloid-necrosome A20-K48-Ub-RIPK1-run113-bridge apoptosis-necroptosis-shift-DAMP-storm HMGB1-TLR4 mtDNA-TLR9-IFN-α IL33-ST2-run099 ATP-P2X7-NLRP3 cDC1-run159 26th-β-cell-death keratinocyte-Loop2 EBV-vICA-necroptosis-ME-CFS necrostatin-1 dabrafenib-RIPK3-Mandal2014 NSA-MLKL necrostatin+A20-combination | run_160*

---

### Extension 154 — FOXO1/FOXO3 β Cell Transcription: PDX1 Nuclear Exclusion, Oxidative Stress Response, Triple Proliferative Arrest

**Date:** 2026-04-12 | **Run:** run_161

**Gap confirmed:** β Cell FOXO1/PDX1 nuclear exclusion/insulin gene regulation: 0 files; run_135 covers only lymphocyte/Treg FOXO1 (PI3Kδ → FOXO1 cytoplasmic → FOXP3); β cell FOXO1 biology entirely absent.

**Core insight:** β cell FOXO1 nuclear translocation under oxidative stress creates **simultaneous β cell dysfunction before death**: FOXO1 nuclear → PDX1 nuclear exclusion → insulin gene ↓ → GSIS failure in living β cells. The directionality is INVERTED versus Tregs: in Tregs (run_135), FOXO1 nuclear = good (FOXP3 maintained); in β cells (run_161), FOXO1 nuclear = bad (PDX1 excluded). This means insulitis Akt suppression simultaneously PRESERVES Treg function (FOXO1 nuclear → FOXP3 ✓) and DESTROYS β cell function (FOXO1 nuclear → PDX1 excluded). A triple proliferative arrest now identified: EZH2/CDK4 (run_157) + mTOR/cyclin D (run_158) + FOXO1/p27/CDK2 (run_161) — all three gates disabled during insulitis. SIRT1 (run_147) deacetylates FOXO1 → FasL/BNIP3 suppressed → third SIRT1 β cell mechanism.

**New mechanisms added (run_161):**
- **FOXO1 nuclear → PDX1 exclusion**: JNK/Akt ↓ → FOXO1 unphosphorylated → nuclear → FOXO1:SIN3A/HDAC at PDX1 targets → insulin gene ↓; Kitamura 2005 Nat Med (FOXO1 nuclear + PDX1 cytoplasmic in insulitis specimens = reciprocal)
- **Directionality paradox**: same Akt suppression → FOXO1 nuclear is beneficial in Tregs (FOXP3 maintained) and harmful in β cells (PDX1 excluded); insulitis creates opposite outcomes in the two critical cell types simultaneously
- **Triple proliferative arrest**: EZH2/p16/CDK4 (run_157) + mTOR/cyclin D/CDK4 (run_158) + FOXO1/p27/CDK2 (run_161) = complete CDK blockade during insulitis; no proliferative gate remains open
- **SIRT1/FOXO1 β cell third mechanism**: SIRT1 deacetylates FOXO1-K242/K245/K262 → FasL/BNIP3 pro-apoptotic output suppressed + MnSOD/catalase survival output maintained; extends run_147 NMN/NR protocol rationale
- **FOXO3 keratinocyte ROS**: FOXO3 + NRF2 → antioxidant response in UV-damaged keratinocytes; FOXO3 nuclear in sun-damaged skin; rs2802292 longevity SNP → β cell aging protection
- **AS1842856 FOXO1 inhibitor**: Langlet 2017 Cell; FOXO1 DBD blockade → PDX1 nuclear release + p27 ↓ → β cell regeneration + dedifferentiation reversal; caution: systemic MnSOD ↓
