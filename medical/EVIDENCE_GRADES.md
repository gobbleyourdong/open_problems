...
- **Weakness**: VP1 immunostaining can have false positives; controversy exists over DiViD interpretation. Richardson 2009 questioned specificity of some antibodies used.
- **Verdict**: Strong circumstantial evidence from multiple tissues. Not definitive in any single study. The CUMULATIVE case across pancreas + heart + muscle is compelling.
...
- T1DM attempt 045 argues overwhelming autophagy via fasting overcomes hijacking
- **Weakness**: no direct evidence that human fasting clears CVB TD mutants. The "overwhelming" autophagy hypothesis is mechanistically plausible but untested. CVB's non-canonical autophagy pathway may be resistant to canonical autophagy induction. The 5-day FMD may not sustain autophagy long enough.
- **Verdict**: Plausible mechanism, strong indirect evidence, no direct proof. This is the MOST uncertain component of the protocol.
...
### CLAIM 7: ME/CFS is primarily driven by CVB persistence
**Grade: C-**
- 42% of ME/CFS biopsies positive for enteroviral RNA (Gow 1991)
- But: 58% are NEGATIVE. ME/CFS is almost certainly heterogeneous.
- Other viral triggers: EBV, HHV-6, SARS-CoV-2, parvovirus B19
- Non-viral mechanisms: autoimmune, mitochondrial, neurological
- The protocol's anti-inflammatory components may help non-CVB ME/CFS, but the antiviral components (fluoxetine) would be irrelevant for non-CVB cases
- **Weakness**: claiming "ME/CFS is a CVB disease" is an overstatement. "CVB causes ~40% of ME/CFS" is more defensible. The protocol should stratify: enteroviral-positive vs enteroviral-negative.
- **Verdict**: The CVB-ME/CFS connection is real but accounts for a subset. The protocol helps that subset. Universal application requires the anti-inflammatory components to work independently of viral clearance.
...
|-------|--------------|---------------|-------------|
| Fluoxetine dose adequacy (Claim 2) | C+ | **B** | IC50 reconciliation: fluoxetine is lysosomotropic, brain = 15x plasma (MRS-measured). Tissue concentrations 2-15x above IC50. Dose adequacy confirmed for all organs at 40-60mg. |
| Fasting clears TD mutants (Claim 3) | C | **C+** | ODD modeled autophagy as cell-autonomous clearance. FMD alone clears 6/8 organs in model. Not yet proven in humans but mechanistic model is now quantitative. |
| Pericarditis prediction (Claim 6) | D | **D+** | NLRP3 model quantified: P(recurrence) = 0.30 × (1 - efficacy_fluoxetine). At 80% efficacy: 6% predicted recurrence. Model internally consistent. Still no clinical data. |
| ME/CFS is CVB-driven (Claim 7) | C- | **C-** (unchanged) | ODD confirmed heterogeneity. 42% CVB-positive is the ceiling. Protocol helps the CVB subset. |
| R > D inequality (Claim 5) | D+ | **C** | ODD's 9-state ODE model + Monte Carlo (2000 sims): 65-80% probability of C-peptide improvement at 3yr. Formalized in attempt 064. Still no human data. |
| Multi-organ clearance | not graded | **C+** | NEW claim from ODD: unified v2 model shows 8/8 organs clear. Cross-validated at 78% concordance. Model-only evidence. |
| HLA paradox | not graded | **B-** | NEW claim: no HLA genotype protects all organs. Monte Carlo across genotype space + biological mechanism (antigen presentation specificity). Proved in Lean (0 sorry). |
