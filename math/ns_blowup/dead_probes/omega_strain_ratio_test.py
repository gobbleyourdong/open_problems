"""
Does |ω|²/|S|² stay large at high |ω|?

This is the remaining gap. If |ω|²/|S|² >> 1 at high |ω|, the pressure
is compressive and the Riccati bound works. If |ω|²/|S|² ≈ 1, dangerous.

Theory: For a vortex tube of radius σ at wavenumber k ~ 1/σ:
  |ω| ~ ω₀ (vorticity at core)
  |S| ~ ω₀/k ~ ω₀σ (strain scales with derivative of velocity)
  |ω|²/|S|² ~ 1/σ² → ∞ as σ → 0

So THINNER tubes have HIGHER ratio. This is GOOD for the proof.

But: at a stagnation point (like TG's origin): |S| = 0, ratio = ∞.
At intermediate points: |S| ~ |ω| is possible.

Test: measure |ω|²/|S|² conditioned on |ω| being in the top 1%, 5%, 10%.
"""
...
def measure_ratio(s, w):
    D=s.D;N=s.N;kd=[s.kx,s.ky,s.kz];u=s.vel(*w)
    S_sq=torch.zeros(N,N,N,dtype=DTYPE)
    for i in range(3):
        for j in range(3):
            Aij=s.ifft(1j*kd[j]*D*u[i])
            Aji=s.ifft(1j*kd[i]*D*u[j])
            Sij=0.5*(Aij+Aji)
            S_sq+=Sij**2
    wf=[s.ifft(D*w[i]) for i in range(3)]
    om_sq=wf[0]**2+wf[1]**2+wf[2]**2
    om=om_sq.sqrt()
    ratio=om_sq/(S_sq+1e-30)
    return om, S_sq, ratio

def main():
    N=32;dt=1e-4
    print("="*70,flush=True)
    print("|ω|²/|S|² AT HIGH |ω|: does it stay large?",flush=True)
    print("="*70,flush=True)
    ...
    for ic_name in ['TG','KP','trefoil']:
        ...
        for pct in [0.99,0.95,0.90,0.80,0.50]:
            print(f"top{(1-pct)*100:4.0f}%  ",end='')
        print(flush=True)

        t=0.
        for epoch in range(12):
            om,S_sq,ratio=measure_ratio(s,w)
            om_flat=om.flatten();ratio_flat=ratio.flatten()
            ...
            for pct in [0.99,0.95,0.90,0.80,0.50]:
                ...
                mask=om_flat>thr
                if mask.sum()>0:
                    mean_ratio=ratio_flat[mask].mean().item()
                    med_ratio=ratio_flat[mask].median().item()
                    line+=f"{mean_ratio:6.1f}  "
                else:
                    line+="  N/A  "
    ...
    # Also: what's the MINIMUM |ω|²/|S|² at the very highest |ω| points?
    print(f"\n\n{'='*70}",flush=True)
    print("MINIMUM |ω|²/|S|² in top 1% |ω| — the WORST CASE",flush=True)
    ...
    for ic_name in ['TG','KP','trefoil']:
        ...
        mask=om_flat>thr99
        r_top=ratio_flat[mask]
        print(f"  {ic_name:>8s}: min={r_top.min().item():.2f}  "
              f"mean={r_top.mean().item():.1f}  "
              f"median={r_top.median().item():.1f}  "
              f"max={r_top.max().item():.1f}  "
              f"|ω|_thr={thr99:.2f}",flush=True)

    print(f"\n{'='*70}",flush=True)
    print("KEY: If min(|ω|²/|S|²) > 1 at top |ω|: pressure is compressive there.",flush=True)
    print("     If min drops to ≈1 or below: strain catches vorticity — DANGEROUS.",flush=True)
    print(f"{'='*70}",flush=True)
