"""Phase scrambler at N=128 — validate j=3,4 in clean inertial range."""
...
def compute_theta(wh,j):
    ...
    Sf=S.reshape(3,3,-1).transpose(2,0,1)
    Si=np.max(np.abs(np.linalg.eigvalsh(Sf)))
...
sample=50  # every 0.05 time units
...
for step in range(10000):  # t=0 to 10
    t=step*dt
    if step%sample==0:
        thetas={j:compute_theta(wh,j) for j in active}
        ...
        results.append({'t':t,'wmax':float(wmax),'thetas':{str(j):float(thetas[j]) for j in active}})
    wh=rk4(wh,dt)
...
with open('ns_blowup/results/phase_scrambler_n128.json','w') as f:
    json.dump({'peaks':{str(j):peaks[j] for j in active},'results':results,'N':N,'nu':nu},f)
print('Saved: ns_blowup/results/phase_scrambler_n128.json',flush=True)
